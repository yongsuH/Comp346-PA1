...
public class Network extends Thread {
	...
	volatile private static int maxNbPackets;                           /* Maximum number of simultaneous transactions handled by the network buffer */
    volatile private static int inputIndexClient, inputIndexServer, outputIndexServer, outputIndexClient;                   /* Network buffer indices for accessing the input buffer (inputIndexClient, outputIndexServer) and output buffer (inputIndexServer, outputIndexClient) */
    volatile private static String clientIP;                            /* IP number of the client application*/
    volatile private static String serverIP;                            /* IP number of the server application */
    volatile private static int portID;                                 /* Port ID of the client application */
    volatile private static String clientConnectionStatus;              /* Client connection status - connected, disconnected, idle */
    volatile private static String serverConnectionStatus;              /* Server connection status - connected, disconnected, idle */
    volatile private static Transactions inComingPacket[];              /* Incoming network buffer */
    volatile private static Transactions outGoingPacket[];              /* Outgoing network buffer */
    volatile private static String inBufferStatus, outBufferStatus;     /* Current status of the network buffers - normal, full, empty */
    volatile private static String networkStatus;    				   /* Network status - active, inactive */
     ...
     */
        ...
        {
        	//
        	//while( ((inputIndexClient +1 ) % maxNbPackets) == outputIndexClient);
        	//
            ...
            if (((inputIndexClient + 1 ) % maxNbPackets) == getoutputIndexServer())
            {	
            ...
            while( ((inputIndexClient + 1 ) % maxNbPackets) == outputIndexServer);
     ...
     */
        ...
        {	
        	while(inputIndexServer == outputIndexClient);
     ...
     */
        ...
        {
        	//
        	//while ( inputIndexClient == outputIndexClient );
        	//
    ...
    {	
    	...
    	while ((clientConnectionStatus != "disconnected") || (serverConnectionStatus != "disconnected"));
